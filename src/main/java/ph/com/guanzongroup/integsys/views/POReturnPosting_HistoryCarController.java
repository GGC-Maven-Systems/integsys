 ...
 * @author Team 1
 */
public class POReturnPosting_HistoryCarController implements Initializable, ScreenInterface {
    ...
    @FXML
    private TextField tfSearchSupplier, tfSearchReferenceNo, tfTransactionNo, tfSupplier, tfReferenceNo, tfPOReceivingNo, tfTransactionTotal, tfDiscountRate, tfDiscountAmount, tfFreightAmt, tfVatRate, tfVatSales, tfVatAmount, tfZeroVatSales, tfVatExemptSales, tfNetTotal, tfBarcode, tfDescription, tfEngineNo, tfCSNo, tfPlateNo, tfFrameNo, tfReceiveQuantity, tfReturnQuantity, tfCost, tfFreightDetail, tfJETransactionNo, tfTotalCreditAmt, tfTotalDebitAmt, tfJEAcctCode, tfJEAcctDescription, tfCreditAmt, tfDebitAmt;
    @FXML
            ...
            if (source instanceof Button) {
                ...
                switch (lsButton) {
                    ...
                    case "btnHistory":
                        ...
                        break;
                    case "btnBrowse":
                        poJSON = poController.PurchaseOrderReturn().searchTransaction(tfSearchSupplier.getText(), tfSearchReferenceNo.getText());
    ...
    private void txtField_KeyPressed(KeyEvent event) {
        try {
            ...
            Logger.getLogger(POReturnPosting_HistoryCarController.class.getName()).log(Level.SEVERE, null, ex);
        }
    }
...
//                }
    ...
    public void loadRecordDetail() {
        try {
            ...
            tfReceiveQuantity.setText(String.valueOf( CustomCommonUtil.setIntegerValueToDecimalFormat(poController.PurchaseOrderReturn().getReceiveQty(pnDetail), false)));
            ...
            tfEngineNo.setText(poController.PurchaseOrderReturn().Detail(pnDetail).InventorySerial().getSerial01());
            tfFrameNo.setText(poController.PurchaseOrderReturn().Detail(pnDetail).InventorySerial().getSerial02());
            tfPlateNo.setText(poController.PurchaseOrderReturn().Detail(pnDetail).InventorySerialRegistration().getPlateNoP());
            tfCSNo.setText(poController.PurchaseOrderReturn().Detail(pnDetail).InventorySerialRegistration().getConductionStickerNo());
        } catch (SQLException | GuanzonException ex) {
            Logger.getLogger(getClass().getName()).log(Level.SEVERE, MiscUtil.getException(ex), ex);
            ShowMessageFX.Error(null, pxeModuleName, MiscUtil.getException(ex));
        }
