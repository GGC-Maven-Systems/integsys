 ...
 * @author Team 2
 */
public class POReplacement_ConfirmationCarController implements Initializable, ScreenInterface {
    ...
    @FXML
    private TextField tfSearchSupplier, tfSearchReferenceNo, tfTransactionNo, tfSupplier, tfTrucking, tfReferenceNo, tfTerm, tfDiscountRate, tfDiscountAmount, tfTotal, tfOrderNo, tfBrand, tfModel, tfModelVariant, tfColor, tfInventoryType, tfMeasure, tfCost, tfOrderQuantity, tfReceiveQuantity, tfAttachmentNo;
    @FXML
    ...
    @FXML
    private TableColumn tblRowNoDetail, tblOrderNoDetail, tblBrandDetail, tblDescriptionDetail, tblCostDetail, tblOrderQuantityDetail, tblReceiveQuantityDetail, tblTotalDetail, tblRowNo, tblSupplier, tblDate, tblReferenceNo, tblRowNoAttachment, tblFileNameAttachment;
    @FXML
            ...
            if (source instanceof Button) {
                ...
                switch (lsButton) {
                    ...
                    case "btnHistory":
                        ...
                        try {
                            poController.PurchaseOrderReceiving().ShowStatusHistory();
                        ...
                        if (selectedFile != null) {
                            ...
                            for (int lnCtr = 0; lnCtr <= poController.PurchaseOrderReceiving().getTransactionAttachmentCount() - 1; lnCtr++) {
                                if (imgPath2.equals(poController.PurchaseOrderReceiving().TransactionAttachmentList(lnCtr).getModel().getFileName())
                                        && RecordStatus.ACTIVE.equals(poController.PurchaseOrderReceiving().TransactionAttachmentList(lnCtr).getModel().getRecordStatus())) {
...
//                            pnAttachment = lnTempRow;
                            pnAttachment = poController.PurchaseOrderReceiving().addAttachment(imgPath2);
                            //Copy file to Attachment path
                            poController.PurchaseOrderReceiving().copyFile(selectedFile.toString());
                            loadTableAttachment.reload();
                    ...
                    case "btnRemoveAttachment":
                        if (poController.PurchaseOrderReceiving().getTransactionAttachmentCount() <= 0) {
                            return;
                        } else {
                            for (int lnCtr = 0; lnCtr < poController.PurchaseOrderReceiving().getTransactionAttachmentCount(); lnCtr++) {
                                if (RecordStatus.INACTIVE.equals(poController.PurchaseOrderReceiving().TransactionAttachmentList(lnCtr).getModel().getRecordStatus())) {
                        ...
                        poJSON = poController.PurchaseOrderReceiving().removeAttachment(pnAttachment);
                        if ("error".equals((String) poJSON.get("result"))) {
                            ShowMessageFX.Warning(null, pxeModuleName, (String) poJSON.get("message"));
                            return;
                        }
    ...
    public void showSerialDialog() {
        ...
        try {
            ...
            try {
                ...
                stageSerialDialog.showDialog((Stage) btnSave.getScene().getWindow(), getClass().getResource("/ph/com/guanzongroup/integsys/views/DeliveryAcceptance_SerialCar.fxml"),
                ...
                if (lbloadImage) {
                    try {
                        ...
                        if (imageinfo_temp.containsKey((String) attachment_data.get(tblAttachments.getSelectionModel().getSelectedIndex()).getIndex02())) {
                            ...
                            // in server
                            if (poController.PurchaseOrderReceiving().TransactionAttachmentList(pnAttachment).getModel().getImagePath() != null && !"".equals(poController.PurchaseOrderReceiving().TransactionAttachmentList(pnAttachment).getModel().getImagePath())) {
                                filePath2 = poController.PurchaseOrderReceiving().TransactionAttachmentList(pnAttachment).getModel().getImagePath() + "/" + (String) attachment_data.get(tblAttachments.getSelectionModel().getSelectedIndex()).getIndex02();
                            } else {
                    ...
                    Platform.runLater(() -> {
                        ...
                        try {
                            ...
                            for (lnCtr = 0; lnCtr < poController.PurchaseOrderReceiving().getDetailCount(); lnCtr++) {
                                ...
                                if (poController.PurchaseOrderReceiving().Detail(lnCtr).getOrderNo() != null && !poController.PurchaseOrderReceiving().Detail(lnCtr).getOrderNo().equals("")) {
                                    cbPreOwned.setSelected(poController.PurchaseOrderReceiving().Detail(lnCtr).PurchaseOrderMaster().getPreOwned());
                                } else {
    ...
    public void initTextFields() {
        ...
        JFXUtil.setFocusListener(txtDetail_Focus, tfOrderNo, tfBrand, tfModel, tfCost, tfReceiveQuantity);
