    ...
    private void handleButtonAction(ActionEvent event) {
        try {
            ...
            switch (lsButton) {
                ...
                case "btnNew":
                    ClearAll();
                    poJSON = poGLControllers.CheckReleases().NewTransaction();
                    if (!"success".equals((String) poJSON.get("result"))) {
                        ShowMessageFX.Warning((String) poJSON.get("message"), psFormName, null);
                        return;
                    }
...
//
            ...
            JFXUtil.setFocusListener(txtField_Focus, tfNote,tfReceivedBy,tfCheckTransNo,tfCheckNo);
            JFXUtil.setFocusListener(txtArea_Focus, taRemarks);
        ...
        if (pnEditMode == EditMode.ADDNEW || pnEditMode == EditMode.UPDATE) {
            ...
            if (event.getClickCount() == 2) {
                ...
                if (loCheckPaym != null) {
                    ...
                    try {
                        poJSON = poGLControllers.CheckReleases().addCheckPaymentToDetail(lsCheckTransNo);
                        if ("success".equals(poJSON.get("result"))) {
                            if (poGLControllers.CheckReleases().getDetailCount() > 0) {
                                pnSelectedDetail = poGLControllers.CheckReleases().getDetailCount() - 1;
        ...
        Task<Void> task = new Task<Void>() {
            ...
            protected Void call() throws Exception {
                try {
                    ...
                    if ("success".equals(poJSON.get("result"))) {
                        if (poGLControllers.CheckReleases().getCheckCount()> 0) {
                            for (int lnCntr = 0; lnCntr < poGLControllers.CheckReleases().getCheckCount(); lnCntr++) {
                                main_data.add(new ModelTableMain(
                                        String.valueOf(lnCntr + 1),
                                        poGLControllers.CheckReleases().poCheckMaster(lnCntr).getTransactionNo() == null ? ""
                                        : poGLControllers.CheckReleases().poCheckMaster(lnCntr).getTransactionNo(),
                                        poGLControllers.CheckReleases().poCheckMaster(lnCntr).getCheckDate() == null ? ""
                                        : CustomCommonUtil.formatDateToShortString(
                                                poGLControllers.CheckReleases().poCheckMaster(lnCntr).getCheckDate()),                                      
                                        poGLControllers.CheckReleases().poCheckMaster(lnCntr).getCheckNo() == null ? ""
                                        : poGLControllers.CheckReleases().poCheckMaster(lnCntr).getCheckNo(),
                                        CustomCommonUtil.setIntegerValueToDecimalFormat(
                                                poGLControllers.CheckReleases().poCheckMaster(lnCntr).getAmount(), true),
                                        "", "", "", "", ""
                                        ));
                            }
                        } else {
