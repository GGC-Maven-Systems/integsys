 ...
 * @author User
 */
public class InvRequest_EntryControllerLP_Food implements Initializable, ScreenInterface {
    ...
    @FXML
    private TextField tfTransactionNo, tfBrand, tfInvType, tfROQ, tfClassification, tfQOH, tfReservationQTY,
...
//                    }
                ...
                case "btnSave":
                    ...
                    if (pnEditMode == EditMode.UPDATE) {
                        invRequestController.Master().setModifiedDate(poApp.getServerDate());
                        invRequestController.Master().setModifyingId(poApp.getUserID());
                    }

                    // Assign modification date to all details
                    for (int lnCntr = 0; lnCntr < detailCount; lnCntr++) {
                        invRequestController.Detail(lnCntr).setModifiedDate(poApp.getServerDate());
                    }

                    // Save Transaction
                    poJSON = invRequestController.isDetailHasZeroQty();
                    if (!"success".equals((String) poJSON.get("result"))) {
    ...
    private void clearDetailFields() {
        /* Detail Fields*/
        CustomCommonUtil.setText("", tfBrand,
                tfReservationQTY, tfQOH, tfInvType,
                tfROQ, tfClassification, tfBarCode, tfDescription);
        ...
        Task<List<ModelInvOrderDetail>> task = new Task<List<ModelInvOrderDetail>>() {
            ...
            protected List<ModelInvOrderDetail> call() throws Exception {
                try {
                    ...
                    // Only add new empty row if in ADDNEW mode and last row is not empty
                    if (pnEditMode == EditMode.ADDNEW && detailCount > 0) {
    ...
    final ChangeListener<? super Boolean> txtArea_Focus = (o, ov, nv) -> {
        ...
        if (!nv) {
            ...
            switch (lsTextAreaID) {
                case "taRemarks":
                    invRequestController.Master().setRemarks(lsValue);
                    break;
            }
        } else {
                ...
                case F3:
                    switch (fieldId) {
                        ...
                        case "tfBarCode":
                            ...
                            poJSON = invRequestController.SearchBarcode(lsValue, true, pnTblInvDetailRow, brandID);
                            ...
                            if ("success".equals(poJSON.get("result"))) {
                                ...
                                try {
                                    currentQty = invRequestController.Detail(pnTblInvDetailRow).getQuantity();
                                } catch (Exception e) {
                                    currentQty = 0.0;
                                }
                        ...
                        case "tfDescription":
                            ...
                            poJSON = invRequestController.SearchBarcodeDescription(lsValue, false, pnTblInvDetailRow, brandID
                            );
                            if ("error".equals(poJSON.get("result"))) {
                                ShowMessageFX.Warning((String) poJSON.get("message"), psFormName, null);
...
//            System.exit(1);
    ...
    private void initTextFieldFocus() {
        List<TextField> loTxtField = Arrays.asList(tfOrderQuantity, tfSearchReferenceNo, tfBarCode, tfDescription, tfSourceNo);//tfReferenceNo, 
